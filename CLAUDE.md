...
- "Keep Downloaded" (pinning) is implemented via the macOS 13+ `contentPolicy` item property (`.downloadEagerlyAndKeepDownloaded` = download eagerly, keep downloading remote updates, never evict; `.inherited` = default) plus two custom context-menu actions declared under `NSExtensionFileProviderActions` in the appex Info.plist, gated by `userInfo` keys (`displayKeepDownloaded` / `displayRemoveKeepDownloaded`). There is no `NSFileProviderItemFields` case for policy changes, so the system is never told directly — instead `performAction` flips the persisted pin set (`pinned-<drive-id>.json` next to the identities file) and nudges the system to re-read the item: `requestDownloadForItem` for files being pinned, `requestModification(of: [.lastUsedDate])` (schedules a `modifyItem` call whose completion returns the fresh item with the new policy) for folders/unpins. Descendants of pinned folders are served with an explicit eager policy (URI-prefix match), so `didUpdate` delivery is what makes the system schedule their downloads.
- `requestDownloadForItem` only works for items the system already knows, and it does not enumerate unbrowsed folders — so pinning a folder records its (remotely listed) subtree in `pending-pinned-<drive-id>.json` and signals the working set; the next `enumerateChanges` delivers those items with the eager policy. The sandboxed extension can NOT write to `~/.cloudreve/fp-events/` (read-only temporary exception) — never persist extension state there; use `~/Library/Application Support/CloudreveFileProvider/` instead. Await all `requestDownloadForItem` retries inside the in-flight XPC call: the service suspends as soon as the completion handler runs, silently killing detached tasks.
