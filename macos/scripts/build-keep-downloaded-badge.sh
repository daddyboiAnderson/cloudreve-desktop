...
GENERATOR="$BUILD_DIR/KeepDownloadedBadge"
...
if [[ ! -x "$GENERATOR" || "$ROOT/macos/scripts/KeepDownloadedBadge.swift" -nt "$GENERATOR" ]]; then
    swiftc -swift-version 5 \
        ...
        -framework ImageIO \
        -o "$GENERATOR" \
        "$ROOT/macos/scripts/KeepDownloadedBadge.swift"
fi

mkdir -p "$OUTPUT_DIR"
"$GENERATOR" "$OUTPUT_DIR/KeepDownloaded.icns"
