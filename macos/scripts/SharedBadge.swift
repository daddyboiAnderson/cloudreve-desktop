...
guard CommandLine.arguments.count == 2 else {
    fputs("usage: SharedBadge <output.icns>\n", stderr)
    exit(EXIT_FAILURE)
}
...
func makeBadge(size: Int) -> CGImage? {
    ...
    let left = CGPoint(x: scale * 0.34, y: scale * 0.5)
    let top = CGPoint(x: scale * 0.68, y: scale * 0.3)
    let bottom = CGPoint(x: scale * 0.68, y: scale * 0.7)
    context.setStrokeColor(CGColor(gray: 1, alpha: 1))
    context.setLineWidth(max(2, scale * 0.09))
    context.setLineCap(.round)
    let connections = CGMutablePath()
    connections.move(to: left)
    connections.addLine(to: top)
    connections.move(to: left)
    connections.addLine(to: bottom)
    context.addPath(connections)
    context.strokePath()

    context.setFillColor(CGColor(gray: 1, alpha: 1))
    let nodeRadius = max(1.5, scale * 0.105)
